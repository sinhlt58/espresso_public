...
import com.digitalpebble.stormcrawler.mongodb.services.LegalUrlService;
...
public class FastURLFilter implements URLFilter/*, JSONResource */{ // conganh comment
    ...
    public String filter(URL sourceUrl, Metadata sourceMetadata,
        ...
        try {
            // conganh add
            String hostname = new URL(urlToFilter).getHost();
            if(sourceMetadata.keySet().contains("key")){
                hostname = "key=" + sourceMetadata.getFirstValue("key");
            }
            getRules(hostname);
            // end conganh
    ...
    void getRules(String hostname){
        ...
        for (LegalUrlEntity legalUrlEntity: legalUrlEntities) {
            Scope scope = new Scope();
            String scopeval = legalUrlEntity.getScope();
            scopeval = scopeval.trim();
            int offset = 0;
            Scope.Type type;
            String value = null;
            ...
            ArrayList<String> patterns = legalUrlEntity.getPatterns();
            for (String patterm: patterns) {
                rlist.add(new Rule(patterm));
            }
