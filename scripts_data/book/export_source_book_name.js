...
const maxDepth = 3;
const excepts = ["tiếng anh"];

let tmpKey = 'tiếng anh'
...
const FOLDER = './' + key.trim().replace(' ', '_') + '/'
...
reqES.searchBookByKeyWithDomain(key, domain).then(async data => {
...
async function getDetailUnit(isTheory, document, book, unit, size) {
    ...
    for (let index = 0; index < detailHits.length; index++) {
        ...
        source.ly_thuyet_or_bai_tap = source.ly_thuyet_or_bai_tap.replace(/https:\/\/img\.\/picture/g, 'https://img.loigiaihay.com/picture')
