...
class Build {
        ...
        for (let index = /*0*/ this._abstract_problem_index + 1; index < this._abstract_solution_index; index++) {
            ...
            if (!is_continue) {
                for (let i = 0; i < this._priority.length; i++) {
                    ...
                    console.log(check, child_priority.type)
    ...
    _smoothTree() {
        ...
        let first_part = this._root.first(node => !node.isRoot() && node.model.type === this._priority[this._check_same_type].type)
        if (first_part) {
        ...
        if(this._abstract_solution_index < this._element.children.length) {
