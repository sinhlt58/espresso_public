...
const DEFAULT_PRIORITY = [
    ...
    },
    {
        ...
        group: 'question',
        is_one_line: false,
        regex: /^(\s{0,}.{0,3}((X|IX|VI{0,3}|IV|I{1,3})[.:]|Phần|PHẦN|TRẮC NGHIỆM|trắc nghiệm|Trắc nghiệm|TỰ LUẬN|tự luận|Tự luận|ĐỌC.{1,3}HIỂU|PHẦN LÀM VĂN|LUYỆN TẬP){1}.{0,25}$)/g,
        is_in_solution: true,
    },
    {
...
class Build {
        ...
        if (first_part) {
            ...
            if (list_invalid_part.length > 0) {
                let new_node = this._tree.parse({
                    numerical_order: this._priority[this._check_same_type].type.numerical_order,
                    text: this._priority[this._check_same_type].type,
                    type: this._priority[this._check_same_type].type.type,
                    indexs: [],
                    sub_indexs: [],
                    plan_indexs: [],
                    solution_indexs: [],
    ...
    _mergeSolutionProblem() {
        this._root_problem.walk({strategy: 'breadth'}, node_problem => {
            if(!node_problem.model.is_merge_wrong) {
                let node_in_solution = this._root_solution.first({strategy: 'breadth'}, node_solution => {
                    // can chinh lai
                    return !node_solution.model.is_merge && node_problem.model.type === node_solution.model.type 
                    // && node_problem.getIndex() === node_solution.getIndex()
                        // (
                        //     ((node_problem.model.is_fixed || node_solution.model.is_fixed) && node_problem.getIndex() === node_solution.getIndex())
                        //     || (!node_problem.model.is_fixed && !node_solution.model.is_fixed && (node_problem.model.label.includes(node_solution.model.label) || node_solution.model.label.includes(node_in_solution.model.indexs)) )
                        // )
                })
                console.log(node_problem.model.text, node_in_solution)
                if(node_in_solution) {
        ...
        // roi tim cac index chua duoc them vao indexs
    }
