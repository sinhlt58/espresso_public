...
docker run --name $container_name -p 8501:$port --mount type=bind,source=$model_location,target=/models/predict_score -e MODEL_NAME=predict_score -t tensorflow/serving 2>&1 > /dev/null &
