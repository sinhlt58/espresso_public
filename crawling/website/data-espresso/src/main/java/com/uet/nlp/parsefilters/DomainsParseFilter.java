...
public class DomainsParseFilter extends ParseFilter {
    ...
    public class CustomRule {
        ...
        private ObjectNode buildJsonNested(Element e, boolean isFisrt) {
            ...
            String spec_header = "";
            Element subUl = null;
            ArrayNode arrayNodes = mapper.createObjectNode().arrayNode();

            if (isFisrt == true && e.children().size() == 1) {

                Element ul = e.child(0);
                Element li = ul.child(0);
                spec_header = li.child(0).text().trim();

                subUl = li.selectFirst("ul");

            } else {

                Element head = e.child(0);
                spec_header = head.text().trim();

                subUl = e.selectFirst("ul");
            ...
            if (subUl != null) {
                for (int i = 0; i < subUl.children().size(); i++) {
                    Element subLi = subUl.child(i);
                    arrayNodes.add(buildJsonNested(subLi, false));
                }
            }
            if (arrayNodes.size() > 0) node.put("children", arrayNodes);
